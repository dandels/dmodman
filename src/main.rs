...
fn main() {
    ...
    if matches.is_present(ARG_UNNAMED) {
        if matches
            ...
            .starts_with("nxm://")
        {
            let _dl_loc = api::nxmhandler::handle_nxm_url(matches.value_of(ARG_UNNAMED).unwrap())
    ...
    if matches.is_present(ARG_QUERY) {
        ...
        let mod_id: u32 = q
            ...
            .to_string()
            .parse()
            .expect("Invalid query. The provided argument must be a valid integer.");
        ...
        let mut fi: api::FileList =
            api::request::get_file_list(&game, &mod_id).expect("Unable to get file info");
